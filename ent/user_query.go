...
// QueryPostsLike chains the current query on the "posts_like" edge.
func (uq *UserQuery) QueryPostsLike() *PostQuery {
	query := &PostQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, selector),
			sqlgraph.To(post.Table, post.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, true, user.PostsLikeTable, user.PostsLikePrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(uq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withPostsLike:  uq.withPostsLike.Clone(),
...
// WithPostsLike tells the query-builder to eager-load the nodes that are connected to
// the "posts_like" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithPostsLike(opts ...func(*PostQuery)) *UserQuery {
	query := &PostQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withPostsLike = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withPostsLike; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*User, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.PostsLike = []*Post{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*User)
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.PostsLikePrimaryKey[1], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{&sql.NullInt64{}, &sql.NullInt64{}}
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "posts_like": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "posts_like" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.PostsLike = append(nodes[i].Edges.PostsLike, n)
			}
		}
	}
