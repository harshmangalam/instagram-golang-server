...
// HasPostsLike applies the HasEdge predicate on the "posts_like" edge.
func HasPostsLike() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(PostsLikeTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2M, true, PostsLikeTable, PostsLikePrimaryKey...),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasPostsLikeWith applies the HasEdge predicate on the "posts_like" edge with a given conditions (other predicates).
func HasPostsLikeWith(preds ...predicate.Post) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(PostsLikeInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2M, true, PostsLikeTable, PostsLikePrimaryKey...),
		)
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
